  ...
  return (
    <>
      <div className="absolute  left-[50%] -translate-x-[50%] w-2/3 h-80 rounded-3xl overflow-hidden shadow-lg bg-white bottom-12">
          ...
          <div className="w-full  h-2/3 p-3">
            ...
            <div className="mt-2">
              {loggedInUser.board.map((board) => (
                <div
                  className="w-full rounded-md px-3 py-2 hover:bg-[#e9e9e9] font-semibold flex gap-3 items-center"
                  key={board?._id}
                  onClick={(e) => {
                    e.preventDefault();
                    e.stopPropagation();
        ...
        <button
          className=" absolute bottom-0 w-full h-14 left-0 text-left pl-5 border-t-[1px] border-gray-300 flex items-center gap-2 font-semibold hover:bg-[#e9e9e9]"
