...
const ShareModal = (props: Props) => {
  ...
  return (
    <div
      className={`w-52  shadow-2xl rounded-lg absolute border-[1px] border-gray-300 p-3 bg-white ${leftTopStyle} z-50`}
    >
      <p className="text-center font-semibold text-lg">{labels?.SHARE}</p>
      <div className="flex items-center gap-5 mt-5 justify-center">
        <button
          className="flex flex-col items-center gap-2"
          onClick={() => {
            window.open(
              `${whatsappWebBaseUrl}${labels?.WHATSAPP_SEND_MESSAGE}%0A${window.location.href}`
            );
            onClose();
          }}
        >
